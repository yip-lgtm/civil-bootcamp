...
|---|---|---|---|
| Engineering design | Engineering design | Core definition | 核心定義 |
| Problem solving | Problem solving | Application | 應用 |
| Communication | Communication | Limitation | 限制 |
| Mathematical formulation | 數學形式 | Key equation | 關鍵方程 |

### 1.2 Key Derivation

For Engineering Computation and Data Science, the fundamental relationships are:
$$f(x) = \text{key equation}, \quad x = \text{variable}$$

This arises from Engineering design applied to the engineering system.
...
```mermaid
graph TD
    A[Input: Engineering design] --> B[Analysis]
    B --> C[Decision]
    C --> D[Approach 1]
    C --> E[Approach 2]
    C --> F[Approach 3]
...
```mermaid
mindmap
    root((Engineering Computation and Data Science))
      Core
        Concepts
      Methods
        Analytical
...
1. **Core concepts** — Engineering design 嘅 foundation
...
## 1. Engineering design — Core concept

### 1.1 Three-process physical essence
For Engineering Computation and Data Science, Engineering design governs the system behavior. Description of Engineering design with bilingual explanation.
...
## 3. Communication — Analysis techniques
...
**Q1.** Derive core relationship for Engineering Computation and Data Science.
**Answer:** Starting from definitions, derive the key equation. Verify units and limits.
