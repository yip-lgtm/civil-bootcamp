...
|---|---|---|---|
| ML for materials | ML for materials | Core definition | 核心定義 |
| Data-driven | Data-driven | Application | 應用 |
| Surrogate | Surrogate | Limitation | 限制 |
| Mathematical formulation | 數學形式 | Key equation | 關鍵方程 |

### 1.2 Key Derivation

For Advanced Mechanics and Materials via Machine Learning, the fundamental relationships are:
$$f(x) = \text{key equation}, \quad x = \text{variable}$$

This arises from ML for materials applied to the engineering system.
...
```mermaid
graph TD
    A[Input: ML for materials] --> B[Analysis]
    B --> C[Decision]
    C --> D[Approach 1]
    C --> E[Approach 2]
    C --> F[Approach 3]
...
```mermaid
mindmap
    root((Advanced Mechanics and Materials via Machine Learning))
      Core
        Concepts
      Methods
        Analytical
...
1. **Core concepts** — ML for materials 嘅 foundation
...
## 1. ML for materials — Core concept

### 1.1 Three-process physical essence
For Advanced Mechanics and Materials via Machine Learning, ML for materials governs the system behavior. Description of ML for materials with bilingual explanation.
...
## 3. Surrogate — Analysis techniques
...
**Q1.** Derive core relationship for Advanced Mechanics and Materials via Machine Learning.
**Answer:** Starting from definitions, derive the key equation. Verify units and limits.
