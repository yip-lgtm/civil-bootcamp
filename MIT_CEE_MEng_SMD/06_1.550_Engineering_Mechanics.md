...
|---|---|---|---|
| Engineering mechanics | Engineering mechanics | Core definition | 核心定義 |
| Continuum | Continuum | Application | 應用 |
| Energy | Energy | Limitation | 限制 |
| Mathematical formulation | 數學形式 | Key equation | 關鍵方程 |

### 1.2 Key Derivation

For Engineering Mechanics, the fundamental relationships are:
$$f(x) = \text{key equation}, \quad x = \text{variable}$$

This arises from Engineering mechanics applied to the engineering system.
...
```mermaid
graph TD
    A[Input: Engineering mechanics] --> B[Analysis]
    B --> C[Decision]
    C --> D[Approach 1]
    C --> E[Approach 2]
    C --> F[Approach 3]
...
1. **Core concepts** — Engineering mechanics 嘅 foundation
...
## 1. Engineering mechanics — Core concept

### 1.1 Three-process physical essence
For Engineering Mechanics, Engineering mechanics governs the system behavior. Description of Engineering mechanics with bilingual explanation.
...
**Q1.** Derive core relationship for Engineering Mechanics.
**Answer:** Starting from definitions, derive the key equation. Verify units and limits.
