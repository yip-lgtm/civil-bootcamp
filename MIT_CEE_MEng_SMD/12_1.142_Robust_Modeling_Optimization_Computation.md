...
|---|---|---|---|
| Robust optimization | Robust optimization | Core definition | 核心定義 |
| Uncertainty | Uncertainty | Application | 應用 |
| Worst case | Worst case | Limitation | 限制 |
| Mathematical formulation | 數學形式 | Key equation | 關鍵方程 |

### 1.2 Key Derivation

For Robust Modeling, Optimization, and Computation, the fundamental relationships are:
$$f(x) = \text{key equation}, \quad x = \text{variable}$$

This arises from Robust optimization applied to the engineering system.
...
```mermaid
graph TD
    A[Input: Robust optimization] --> B[Analysis]
    B --> C[Decision]
    C --> D[Approach 1]
    C --> E[Approach 2]
    C --> F[Approach 3]
...
```mermaid
mindmap
    root((Robust Modeling, Optimization, and Computation))
      Core
        Concepts
      Methods
        Analytical
...
1. **Core concepts** — Robust optimization 嘅 foundation
...
## 1. Robust optimization — Core concept

### 1.1 Three-process physical essence
For Robust Modeling, Optimization, and Computation, Robust optimization governs the system behavior. Description of Robust optimization with bilingual explanation.
...
## 3. Worst case — Analysis techniques
...
**Q1.** Derive core relationship for Robust Modeling, Optimization, and Computation.
**Answer:** Starting from definitions, derive the key equation. Verify units and limits.
