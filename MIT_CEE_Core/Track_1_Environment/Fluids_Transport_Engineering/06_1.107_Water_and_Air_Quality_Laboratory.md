...
| Chemistry | Chemistry | Application | 應用 |
| Biology | Biology | Limitation | 限制 |
| Mathematical formulation | 數學形式 | Key equation | 關鍵方程 |

### 1.2 Key Derivation

For Environmental Chemistry Laboratory, the fundamental relationships are:
...
```mermaid
mindmap
    root((Environmental Chemistry Laboratory))
      Core
        Concepts
      Methods
        Analytical
...
For Environmental Chemistry Laboratory, Lab methods governs the system behavior. Description of Lab methods with bilingual explanation.
...
**Q1.** Derive core relationship for Environmental Chemistry Laboratory.
**Answer:** Starting from definitions, derive the key equation. Verify units and limits.
