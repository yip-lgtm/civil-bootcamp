...
|---|---|---|---|
| Operations analytics | Operations analytics | Core definition | 核心定義 |
| Queueing | Queueing | Application | 應用 |
| Optimization | Optimization | Limitation | 限制 |
| Mathematical formulation | 數學形式 | Key equation | 關鍵方程 |

### 1.2 Key Derivation

For Business and Operations Analytics, the fundamental relationships are:
$$f(x) = \text{key equation}, \quad x = \text{variable}$$

This arises from Operations analytics applied to the engineering system.
...
```mermaid
graph TD
    A[Input: Operations analytics] --> B[Analysis]
    B --> C[Decision]
    C --> D[Approach 1]
    C --> E[Approach 2]
    C --> F[Approach 3]
...
```mermaid
mindmap
    root((Business and Operations Analytics))
      Core
        Concepts
      Methods
        Analytical
...
1. **Core concepts** — Operations analytics 嘅 foundation
...
## 1. Operations analytics — Core concept

### 1.1 Three-process physical essence
For Business and Operations Analytics, Operations analytics governs the system behavior. Description of Operations analytics with bilingual explanation.
...
## 3. Optimization — Analysis techniques
...
**Q1.** Derive core relationship for Business and Operations Analytics.
**Answer:** Starting from definitions, derive the key equation. Verify units and limits.
